    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            for (var i = 0; i < data.rows.length; i++) {
                ...
                var tpl = '<tr><td><span class="text-muted bg-info" style="cursor:pointer" onclick="$(\'#log0' + (item.id.replace('-', '')) + '\').toggle()">' + (item.path) + '</span><br><span class="text-info bg-warning">' + (item.trace) + '</span></td><td class="text-info vmiddle">' + (item.request) + '</td><td class="text-muted vmiddle">' + (item.time) + '</td><td class="vmiddle"><button type="button" class="btn btn-xs btn-danger" page="1" onclick="log0Delete(this,\'' + (item.id) + '\')">删除</button></td></tr>';
                tpl += '<tr id="log1' + (item.id.replace('-', '')) + '" style="display:none"><td colspan="4" align="left"><pre>' + (item.response) + '</pre></td></tr>';
                //console.log(tpl);
                tbl.append($(tpl));
            }
        },
