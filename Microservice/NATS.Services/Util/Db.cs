...
namespace NATS.Services.Util
{
    ...
    public class DbTable<T> : SimpleClient<T> where T : class, new()
    {
        ...
        public DbTable(ISqlSugarClient context = null, string connectionString = null, string separatorChars = ":", Action<string> action = null) : base(context)
        {
            if (context != null || connectionString == null) return;
            Context = connectionString.NewSqlSugarClient(separatorChars, action);
        }
    }
    /// <summary>
