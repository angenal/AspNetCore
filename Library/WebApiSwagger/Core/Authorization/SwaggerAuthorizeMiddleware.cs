...
namespace WebApiSwagger.Core.Authorization
{
    /// <summary>
    /// Swagger 授权中间件
    /// </summary>
    internal class SwaggerAuthorizeMiddleware
    {
        ...
        /// Swagger 接口文档选项配置
        /// </summary>
        private readonly SwaggerDocOptions _options;
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (_options.EnableAuthorization() && path.StartsWith($"/{_options.RoutePrefix}"))
            {
                if (path == $"/{_options.RoutePrefix}/login.html")
                {
                    ...
                    if (method == "post")
                    {
                        var user = new SwaggerAuthorizationUser(context.Request.Form["userName"], context.Request.Form["password"]);
                        if (!_options.SwaggerAuthorizations.Any(x => x.UserName == user.UserName && x.Password == user.Password))
                        {
                        ...
                        context.Response.Cookies.Append(SWAGGER_AUTH_COOKIE, user.Token, new CookieOptions() { Expires = DateTimeOffset.Now.AddMonths(1) });
                ...
                if (!_options.SwaggerAuthorizations.Any(x => !string.IsNullOrWhiteSpace(x.Token) && x.Token == context.Request.Cookies[SWAGGER_AUTH_COOKIE]))
                {
                    context.Response.Redirect($"/{_options.RoutePrefix}/login.html");
                    return;
                }
