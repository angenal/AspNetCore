namespace WebCore.Crawler
{
    public class AbotX
    {
        /// <summary>
        /// AbotX.lic base64 string.
        /// </summary>
        //const string License = "";

        //internal static void Load()
        //{
        //    string p = "Basic", v = License.FromBase64String();
        //    Type t = Assembly.GetAssembly(typeof(AbotX2.Crawler.CrawlerX)).GetType("AbotX2.Util.InMemoryLicense", true);
        //    FieldInfo field = t.GetField(p, BindingFlags.NonPublic | BindingFlags.Static);
        //    field.SetValue(null, v);
        //}

        ///// <summary>
        ///// Read from base64String license.
        ///// </summary>
        //static Stream GetLicense(string base64String) => new MemoryStream(Convert.FromBase64String(base64String));
    }
}
