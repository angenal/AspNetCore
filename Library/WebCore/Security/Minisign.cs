...
using System.Text;
using WebCore.Security.Exceptions;
using WebCore.Security.Models;
...
namespace WebCore.Security
{
    ...
    public static class Minisign
    {
            ...
            string trustedComment = "", string outputFolder = "")
        {
            ...
            var binarySignature = minisignSignature.SignatureAlgorithm.ConcatArrays(
            ...
            var globalSignature =
                PublicKeyAuth.SignDetached(
                    minisignSignature.Signature.ConcatArrays(Encoding.UTF8.GetBytes(trustedComment)),
            ...
            string outputFolder = "", string keyPairFileName = "minisign")
        {
            ...
            var checksum =
                GenericHash.Hash(
                    minisignPrivateKey.SignatureAlgorithm.ConcatArrays(keyId, keyPair.PrivateKey), null,
                    32);
            minisignPrivateKey.KeyId = keyId;
            minisignPrivateKey.SecretKey = keyPair.PrivateKey;
            minisignPrivateKey.Checksum = checksum;

            var dataToProtect = keyId.ConcatArrays(keyPair.PrivateKey, checksum);
            ...
            var encryptedKeyData = dataToProtect.Xor(encryptionKey);
            ...
            if (writeOutputFiles)
            {
                ...
                var binaryPublicKey = minisignPublicKey.SignatureAlgorithm.ConcatArrays(
                ...
                var binaryPrivateKey = minisignPrivateKey.SignatureAlgorithm.ConcatArrays(
        ...
        public static bool ValidateSignature(string filePath, MinisignSignature signature, MinisignPublicKey publicKey)
        {
            ...
            if (!signature.KeyId.ConstantTimeEquals(publicKey.KeyId)) return false;
            ...
            if (PublicKeyAuth.VerifyDetached(signature.Signature, file, publicKey.PublicKey))
            {
                ...
                return PublicKeyAuth.VerifyDetached(signature.GlobalSignature,
                    signature.Signature.ConcatArrays(signature.TrustedComment), publicKey.PublicKey);
            }
        ...
        public static MinisignSignature LoadSignature(byte[] signature, byte[] trustedComment, byte[] globalSignature)
        {
            ...
            var minisignSignature = new MinisignSignature
            {
                SignatureAlgorithm = signature.SubArray(0, 2),
                KeyId = signature.SubArray(2, 8),
        ...
        public static MinisignPublicKey LoadPublicKey(byte[] publicKey)
        {
            ...
            var minisignPublicKey = new MinisignPublicKey
            {
                SignatureAlgorithm = publicKey.SubArray(0, 2),
                KeyId = publicKey.SubArray(2, 8),
                PublicKey = publicKey.SubArray(10)
            };
        ...
        public static MinisignPrivateKey LoadPrivateKey(byte[] privateKey, byte[] password)
        {
            ...
            var minisignPrivateKey = new MinisignPrivateKey
            {
                SignatureAlgorithm = privateKey.SubArray(0, 2),
                KdfAlgorithm = privateKey.SubArray(2, 2),
                ChecksumAlgorithm = privateKey.SubArray(4, 2),
                KdfSalt = privateKey.SubArray(6, 32),
                KdfOpsLimit = BitConverter.ToInt64(privateKey.SubArray(38, 8), 0), //currently unused
                KdfMemLimit = BitConverter.ToInt64(privateKey.SubArray(46, 8), 0) //currently unused
            };
            ...
            var encryptedKeyData = privateKey.SubArray(54, 104);

            var decryptionKey = PasswordHash.ScryptHashBinary(password, minisignPrivateKey.KdfSalt,
                PasswordHash.Strength.Sensitive,
                104);

            var decryptedKeyData = encryptedKeyData.Xor(decryptionKey);
            minisignPrivateKey.KeyId = decryptedKeyData.SubArray(0, 8);
            minisignPrivateKey.SecretKey = decryptedKeyData.SubArray(8, 64);
            minisignPrivateKey.Checksum = decryptedKeyData.SubArray(72, 32);
            ...
            var calculatedChecksum = GenericHash.Hash(minisignPrivateKey.SignatureAlgorithm.ConcatArrays(minisignPrivateKey.KeyId,
                        minisignPrivateKey.SecretKey), null, 32);

            if (!minisignPrivateKey.Checksum.ConstantTimeEquals(calculatedChecksum))
            {
