    ...
    public class TextMessages
    {
        ...
        public IResponseMessageBase GetResponseMessage(RequestMessageText requestMessage, CustomMessageHandler handler)
        {
            ...
            var h = requestMessage.StartHandler().Default(() =>
            {
                ...
                //defaultResponseMessage.Content = "success";
                //return defaultResponseMessage;

                return new SuccessResponseMessage();
            });

            foreach (string keyword in Data.MP.TextRequestReply.Keys)
            {
                if (string.IsNullOrEmpty(keyword)) continue;
                DTO.RequestReplyDTO model = Data.MP.TextRequestReply[keyword];
                h.Keyword(keyword, () =>
                {
