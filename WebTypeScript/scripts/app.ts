...
declare class JQuery { // *类型定义new JQuery(el)
    length: number;
    constructor(el: HTMLElement);
    count(): number;
}
declare namespace PersonLib.Options { // *Lib.Options for Organizing Types
    /** new Person().Say() */
    interface Say { }
    /** new Person().Alert({message:'hello'}) */
    interface Alert { message: string; model?: boolean; }
}
/** 申明 当前域变量 */
